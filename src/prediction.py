...
def predict_and_plot(filename: str, model=None):
    ...
    # Step 2: Prepare feature set for prediction
    feature_columns = ['SMA_20', 'SMA_50', 'EMA_20', 'Higher_High', 'Lower_Low', 'Supply_Zone', 'Demand_Zone', 'Local_Max', 'Local_Min']
    ...
    try:
        # Predict binary movement (up/down) and map to price change
    ...
    data['Actual_Movement'] = np.where(data['Close'] > data['Close'].shift(1), 1, 0)  # 1 if price went up, 0 if down
...
def plot_results(data):
    ...
    mpf.plot(data.set_index(data.index), type='candle', volume=True, style='yahoo')
